...
public class ReticlePoser : MonoBehaviour
{
    ...
    protected virtual void LateUpdate()
    {
        ...
        if ((showOnHitOnly && !result.isValid) || pointCount <= 1)
        {
            ...
            if (Spielmodi.spielmodi == 2)
            {
                foreach (GameObject o in createField.asteroidList)
                {
        ...
        if (result.isValid)
        {
            ...
            if (Spielmodi.spielmodi == 2)
            {
                ...
                if (hitTarget.gameObject.name == "Cube(Clone)")
                {
                    c = hitTarget.GetComponent<Renderer>().material.color;

                    Debug.Log(c);
                }
                if (hitTarget.gameObject.name == "Cube(Clone)" && c == Color.green && (input.isTriggerJumped || input.isTriggerJumpLeft))
                {
                    playSound(clipDestroyed);

                }
                else if(hitTarget.gameObject.name == "Cube(Clone)" && c != Color.green && (input.isTriggerJumped || input.isTriggerJumpLeft))
                {
                    playSound(clipWrongDestroyed);

                }


                if (hitTarget.gameObject.name == "Cube(Clone)" && (input.isTriggerJumped || input.isTriggerJumpLeft))
                {
                    createField.asteroidList.Remove(hitTarget.gameObject);
                    DestroyImmediate(hitTarget.gameObject);



                }

                if (hitTarget.gameObject != null)
                {
                    if (hitTarget.gameObject.name == "Cube(Clone)")
                    {
                        playSound(clip);

                        outline = hitTarget.gameObject.GetComponent<Outline>();
